...
**Highlights:** shader-lifecycle hardening release. Fixes a Windows + Chrome crash on machines with NVIDIA Optimus dual-GPU laptops, where switching between the integrated and discrete GPU loses the WebGL context, the renderer auto-destroys every `GLShader` via the existing `ONCONTEXT_LOST` subscription, and `gl.deleteProgram` throws on the dead context (ANGLE / D3D11 is strict about cross-context object use; Apple's GL driver silently no-ops, which is why the bug was Windows-only). The throw left `GLShader.uniforms = null` while `ShaderEffect.enabled` stayed true, and the next frame's `setUniform("uTime", …)` then crashed with `TypeError: Cannot read properties of null (reading 'uTime')`. The fix is twofold: (1) `GLShader.destroy()` is now atomic + idempotent + wraps `gl.deleteProgram` in a try / catch so a partial-destroy can't happen, and (2) `ShaderEffect.destroy()` sets `enabled = false` BEFORE the inner destroy so the public guard is in the safe state even if the inner destroy throws. The same release also completes the previously half-implemented `ONCONTEXT_RESTORED` recovery path — shaders now suspend on context lost (keep source + cached uniform values) and recompile + replay uniforms on context restored, so the visual state survives a GPU switch transparently.

### Added
- `GLShader.destroyed: boolean` — public read-only flag that flips `true` once `destroy()` has been called. Every method on the shader silently no-ops after that (defensive against still-registered `GAME_UPDATE` handlers calling `setTime(...)` on a stale reference). Idempotent — calling `destroy()` twice is safe.
- `GLShader.suspended: boolean` — public read-only flag that's `true` between an `ONCONTEXT_LOST` and the matching `ONCONTEXT_RESTORED`. The GL program / attributes / uniforms are released during the suspended window but the shader source code is preserved so the shader can be transparently rebuilt against the new context.
- **`GLShader` survives a context lost → restored cycle** with uniform values intact. Every uniform value written through `setUniform` is cached on the shader; on `ONCONTEXT_RESTORED` the shader recompiles against the new GL context and replays the cached values against the freshly-extracted uniforms proxy. User code does not have to re-apply uniforms after a GPU switch. Completes the design intent the existing `ONCONTEXT_LOST` subscription's comment already documented ("will be recreated on context restore") — only half of which was previously implemented.
- `ShaderEffect.destroyed: boolean` — public read-only flag, distinct from `enabled` (which also toggles transiently across a context lost / restored cycle), to give callers a stable signal for "this effect has been explicitly released".
- `ShaderEffect` subscribes to `ONCONTEXT_LOST` / `ONCONTEXT_RESTORED` and flips `enabled` accordingly. The renderer's `beginPostEffect` filter (`fx.enabled !== false`) now skips the effect entirely during the suspended window, instead of trying to bind a null program.

### Changed
- **`platformer` and `platformer-matter` example coins migrated to per-instance `ShineEffect`** — previously each example kept a module-level singleton `coinShader` with a manual refcount tracked via `LEVEL_LOADED` / `onDestroyEvent`. The other 19.5+ shader-using examples (`plinko-planck/entities/peg.ts`, `dropZone.ts`) already used per-instance shaders + `enabled = true / false` toggling for visibility — the platformer coins were the deviation in 19.5.0. Both platformer entities now follow the same pattern.

### Fixed
- **`GLShader.destroy()` is atomic + idempotent** — wraps `gl.deleteProgram` in a try / catch so a throw on a dead ANGLE / D3D11 context can't leave the shader in a half-destroyed state, and an early-return on the new `destroyed` flag makes double-destroy a no-op. The pre-19.6 implementation set `uniforms = null` before calling `gl.deleteProgram`, then nulled the remaining fields after — if the GL call threw, the shader was left with `uniforms === null` and `program !== null`, the partial state that caused the 19.5.0 Windows crash.
- **`ShaderEffect.destroy()` reorders `enabled = false` BEFORE `_shader.destroy()`** — defense in depth. Even if the inner destroy throws outward (or any future failure mode leaves the inner shader half-torn-down), the public `setUniform` / `bind` / `getAttribLocation` / `uniforms` / `attributes` paths immediately become no-ops via the existing `enabled` gate.

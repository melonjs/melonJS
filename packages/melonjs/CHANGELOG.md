...
- **Breaking:** `Light2d` constructor `x` / `y` now denote the light's **center** (matching `Ellipse(x, y, w, h)` conventions), not the bounding-box top-left. Existing call sites that passed top-left coordinates need to add `radiusX` / `radiusY`: `new Light2d(x, y, r)` → `new Light2d(x + r, y + r, r)`. Code that already uses `light.centerOn(x, y)` to position lights is unaffected. Note: the bounding box now extends symmetrically into negative-relative coordinates around `pos`, so e.g. `new Light2d(0, 0, 25)` has `bounds.x === -25`.

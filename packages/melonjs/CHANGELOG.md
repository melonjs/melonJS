...
### Added
- **Experimental WebGPU renderer** — requestable as `renderer: video.WEBGPU` or via the `#webgpu` URI fragment. What exists today is the backend's asynchronous bootstrap — `navigator.gpu` adapter/device negotiation and canvas configuration inside `await app.init()`, which is the workload the new two-phase `Application` startup exists for — plus a real per-frame clear pass; every other drawing method is still a no-op, so a WebGPU application boots, runs its loop and clears to its background color, and renders nothing else. **Deliberately excluded from `video.AUTO`**, and it stays opt-in until the backend reaches feature parity with WebGL, so no existing game can be moved onto a partial backend by a browser gaining support; `app.init()` rejects when WebGPU is unavailable rather than quietly substituting another renderer — someone asking for WebGPU is testing WebGPU. `WebGPURenderer` is exported, reports `type: "WebGPU"` / `shaderLanguage: "wgsl"`, and its capability flags honestly stay `false` until each path lands. See the new **Hello WebGPU** example
...
### Changed (breaking)
- **Starting a game is now two steps: construct the `Application`, then `await app.init()`** — and calling `init()` is **mandatory**, not optional. The constructor resolves the settings, creates the game world and registers the DOM event bridging; the new argument-less `init()` builds the renderer and everything that depends on it (the canvas, the initial resize layout, the console banner). This is what allows a renderer to be acquired asynchronously — WebGPU cannot create a device synchronously — while `init()` still resolves without suspending on the Canvas and WebGL backends. Renderer failures surface as a rejection of `init()` (e.g. `renderer: video.WEBGL` on a device without WebGL 2), no longer as a constructor throw. The pre-created bootstrap application and the `legacy` setting are removed with it; the exported `game` now names the most recently **initialized** `Application` — it is set at the end of `init()`, so it never points at a half-built app, and it is `undefined` until the first `init()` resolves:
  ```js
  // before                                    // after
  video.init(640, 480, { parent: "screen" });  const app = new Application(640, 480, { parent: "screen" });
                                               await app.init();
  ```
  Two lifecycle notes that come with the split: `app.destroy()` is now **terminal** — a destroyed Application cannot be re-initialized (`init()` rejects; construct a new instance instead), and a repeated `init()` on a live app is a warned no-op. And renderers gained an argument-less async `init()` lifecycle hook that `Application#init` awaits after construction — the base `Renderer` provides a resolve-immediately default, so custom renderer classes extending it need no changes; a custom renderer that already had its own `init()` method should expect the engine to call it with no arguments

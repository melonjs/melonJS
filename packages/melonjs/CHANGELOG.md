...
### Fixed
- Lit meshes: specular highlights sat in the wrong place under a scaled ancestor ([#1636](https://github.com/melonjs/melonJS/issues/1636)). The shader needs the camera's position for the Blinn-Phong half vector, and it was derived from the view matrix as `-Rᵀ·t` — which is the right point only when the upper 3×3 is orthonormal. That matrix is not the camera's view alone: `Container.draw` folds every ancestor into it, so one scaled container — a model in an inventory panel scaled to fit, a diorama, a grow/shrink animation — put the eye somewhere else and took the highlight with it. Counter-intuitively a *uniform* scale is the worst case, not the benign one: the extraction yields `s·e` where the truth is `e/s`, so at half scale the eye lands four times too close. It is now the translation column of the view's inverse, which needs no assumption about the matrix. Diffuse lighting, geometry and every unlit path were unaffected either way
- Lit meshes: specular lighting was wrong on whichever tier drew second in a frame, and a mesh with an alpha map could lose its cutout the same way. A uniform belongs to its program, and the instanced and non-instanced tiers are two programs sharing one batcher — but the batcher's skip-the-redundant-upload cache was not dropped when the program changed under it, so the second tier was never sent those values at all and ran on whatever its own program happened to hold. The failure hid behind the sentinels: an untouched GL uniform reads back as 0, and a cache holding a value the fresh program already defaults to agrees with it by accident. `uShininess` and `uSpecular` default to 0, which is exactly "no highlight", so an instanced crowd behind a lit prop at the same shininess lost its specular **outright** rather than misplacing it; the camera position defaults to `(0, 0, 0)`, so the other tier shaded its highlight from the world origin and it shifted as the camera moved; and `uHasAlphaMap` defaults to 0, so instanced foliage rendered as opaque rectangles instead of cut-out leaves. Every value the material and placement paths push is now re-issued on a program change. Diffuse lighting, unlit meshes and any material at `shininess` 0 with no alpha map were unaffected. Present since 20.0.0
...
- Loader: `loader.crossOrigin` and `loader.withCredentials` are documented as read-only, which they always were — they are module bindings, so the assignment both JSDoc examples showed threw a `TypeError`. Use `loader.setOptions({ … })`, the same switch for every asset type

...
### Added
- **3D collision: the `Box3d` body shape** ([#1476](https://github.com/melonjs/melonJS/issues/1476)) — a body can now be resolved along **Z** as well as X and Y, which no shape could express before. Every other built-in shape (`Polygon`, `Rect`, `RoundRect`, `Ellipse`) is planar and goes through the 2D SAT narrowphase, whose minimum translation vector is a `Vector2d`: a depth gate could filter candidate *pairs*, but nothing could ever produce a Z pushback, so a character in a 2.5D scene could be pushed around the screen plane and never into or out of it. A `Box3d` pair is instead resolved by an AABB-vs-AABB narrowphase (`testBox3dBox3d`) that also fills the new `ResponseObject.overlapZ` / `overlapNZ`, and `Body` gains `velZ`, `forceZ`, `frictionZ` and `maxVelZ` to move along that axis. `raycast3d` gets an exact ray-vs-AABB slab test for bodies carrying a `Box3d`, replacing a bounding-sphere approximation whose radius came from a **2D** bounds with no depth at all — a wide floor slab read as an enormous sphere and reported a hit well above its actual surface, which is what made it useless for probing floor height. **Nothing about the 2D path changes.** Z arrives as scalars beside `overlapV` / `overlapN` / `body.vel` rather than by widening them, because `Vector3d` is not a subclass of `Vector2d` and retyping them would break every existing consumer; the minimum translation axis is a single axis, so a planar pair leaves every Z field at `0` and the arithmetic is inert. `Box3d` may also be mixed with planar shapes: the planar shape is treated as **unbounded along Z** (an infinitely extruded prism of its own outline), so introducing one `Box3d` body into an existing 2D game leaves its world shapes colliding exactly as before. Use `collisionType` / `collisionMask` to opt specific shapes out.
...
### Fixed
- **untextured glTF materials rendered washed out and desaturated** — `pbrMetallicRoughness.baseColorFactor` is defined by the glTF spec in **linear** space, while a melonJS `tint` is 8-bit sRGB — the same space as a CSS colour or a PNG texel. The loader scaled the linear number by 255 and used it directly, so an authored mid-green (linear `0.29`) reached the renderer as sRGB `0.58` and came out pale mint. Every solid-colour glTF material was affected; the brighter the authored colour, the smaller the error, which is why it read as "slightly off" rather than obviously broken. The factor is now encoded through the sRGB transfer function (`linearToSrgb8`), which also clamps the domain — an exporter emitting a slightly out-of-range factor would previously have reached `Math.pow` with a negative base and NaN-ed the whole tint.
- **an indexed (palette) PNG texture in a glTF asset rendered greyscale on Safari** — the glTF parser decoded images to an `HTMLImageElement`, unlike the ordinary image loader, which has always produced an `ImageBitmap`. That difference is invisible for an RGBA source but not for an **indexed** one (PNG `colorType` 3): WebKit's `copyExternalImageToTexture` uploads such an image as its raw palette *indices* rather than expanding them through the palette, so the texture arrives with `r == g == b` — geometry and animation perfectly intact, all colour gone. Other browsers normalise the image at decode and were unaffected, and the WebGL backend's `texImage2D` never had the problem, so it presented as a WebGPU regression in one browser only. Nothing in the WebGPU API can correct this at upload time — `flipY`, `premultipliedAlpha` and `colorSpace` are the only knobs, and expanding a palette belongs to the decoder — so glTF images now decode to an `ImageBitmap`, which is RGBA by definition. The decode falls back to the previous element path on failure, since `createImageBitmap` is stricter about malformed sources and a stricter loader would itself be a regression.
- **`Body.destroy()` threw on any shape class not registered with the legacy object pool** — the shape-release loop fell through to `pool.push`, which throws for an unregistered class. Because the body's bounds had already been released one line earlier, the throw aborted `destroy()` half-finished and left the body holding a recycled `Bounds`; the failure then surfaced much later as a `TypeError` deep in the broadphase, nowhere near the cause. Reachable with any user-defined shape type.
- **an unknown shape-type pair crashed the physics step** — `Detector.collides` indexed its lookup table by concatenating the two shape type names and called the result immediately, so a combination with no entry threw a `TypeError` mid-step and took the whole world update with it. Reachable today with any user-defined shape type. An unlisted pair now warns once and is treated as "no collision": a missed contact is recoverable, a thrown physics step is not.

...
### Added
- **`exclusion` blend mode on both GPU backends** ([#1318](https://github.com/melonjs/melonJS/issues/1318)) — the one CSS advanced blend mode that needs no shader. `exclusion(a, b) = a + b - 2ab` has no per-pixel branch, so it falls straight out of fixed-function blending as `src*(1-dst) + dst*(1-src)` — `blendFunc(ONE_MINUS_DST_COLOR, ONE_MINUS_SRC_COLOR)` on WebGL 2 and the matching `one-minus-dst` / `one-minus-src` factors on WebGPU. No render target, no second pass. Exact for an opaque source and approximate for a translucent one, the same caveat `screen` already carries. The remaining six advanced modes still need shaders
- **Per-shape collision settings** ([#1590](https://github.com/melonjs/melonJS/issues/1590)) — a body built from several shapes no longer has to treat them all alike. Each shape may carry its own `collisionType` / `collisionMask`, plus `isActive` (default `true`; `false` removes it from collision entirely — no test, no contact, no events — without removing it from the body) and `isTrigger` (default `false`; `true` still collides and still fires events, but skips the position correction, i.e. a per-shape `isSensor`). So a character can have a footprint that only collides with terrain and a hurtbox that only reacts to attacks, on one body, instead of being split across two objects. Unset fields inherit the body's, so **a shape that sets nothing behaves exactly as before**; a shape can only NARROW what its body allows, never widen it. A trigger removes only itself from the solver — a solid shape on the same body still pushes out, and the outcome does not depend on the order shapes were added (`response.isTriggerContact` reports whether a contact exists only through triggers). `isActive: false` also hides the shape from raycasts; it deliberately does not shrink the body's `bounds`, so toggling never resizes it in the broadphase. Collision handlers can tell which shape was hit — the response's `indexShapeA` / `indexShapeB` index into `body.shapes`, and under the modern handlers they are swapped per receiver alongside `a`/`b`, so `indexShapeA` is always your own shape. Honoured by the builtin adapter and by `@melonjs/planck-adapter` (which maps them onto Box2D's native per-fixture filters); `@melonjs/matter-adapter` filters per body only and ignores them
...
### Changed (breaking)
- **`video.AUTO` (the default) now prefers WebGPU: the ladder is WebGPU → WebGL 2 → Canvas** — on a WebGPU-capable browser an existing game using `AUTO` starts on the WebGPU backend, which renders the entire engine feature set identically to WebGL (all 44 examples verified side-by-side). `await app.init()` still always resolves under AUTO — the WebGPU adapter/device negotiation is awaited first and falls through to the synchronous WebGL/Canvas candidates when it rejects. Pin `renderer: video.WEBGL` (or the `#webgl` URI fragment) to keep a game on WebGL
- **`Batcher` is now the backend-neutral base class, and the WebGL base batcher is renamed `WebGLBatcher`** — the shared `Batcher` base defines the lifecycle contract every batcher honors (`init` / `bind` / `unbind` / `flush` / `reset` / `destroy`), with `WebGLBatcher` realizing it on GL state and the newly exported `WebGPUBatcher` (plus `WebGPUQuadBatcher` / `WebGPUPrimitiveBatcher`) on WebGPU render passes. Custom WebGL batchers change one word — extend `WebGLBatcher` instead of `Batcher`; constructor signature, settings and every method are unchanged. `renderer.addBatcher()` now rejects up front any batcher that does not extend that backend's base class (`WebGLBatcher` / `WebGPUBatcher`), instead of failing mid-draw later
- **Starting a game is now two steps: construct the `Application`, then `await app.init()`** — and calling `init()` is **mandatory**, not optional. `init()` is asynchronous because a WebGPU device cannot be acquired synchronously; it still resolves without suspending on the Canvas and WebGL backends. Renderer failures surface as a rejection of `init()` (e.g. `renderer: video.WEBGL` on a device without WebGL 2), no longer as a constructor throw. The pre-created bootstrap application and the `legacy` setting are removed with it; the exported `game` now names the most recently **initialized** `Application` — it never points at a half-built app, and it is `undefined` until the first `init()` resolves:
  ```js
  // before                                    // after
...
- an attribute declared without an explicit `offset` is now packed after the previous one instead of defaulting to byte 0. Layouts that omitted offsets were previously overlapping every attribute at 0 and reading the wrong data; if you relied on that, pass explicit offsets ([#1551](https://github.com/melonjs/melonJS/issues/1551))
- a `Batcher` whose vertex stride is not a multiple of 4 bytes now throws at construction. Previously it built with a fractional vertex size and silently discarded every vertex write ([#1551](https://github.com/melonjs/melonJS/issues/1551))
...
### Fixed
- **WebGPU reported `add` and `lighter` as unsupported blend modes** — `setBlendMode` returns the mode actually applied, which callers compare against what they requested to detect a fallback. WebGPU returned the *canonical* name, so the aliases `add` and `lighter` both came back as `"additive"` and read as rejected — the Blend Modes example showed them green under WebGL and red under WebGPU. Both backends always rendered them identically; only the report was wrong. WebGPU now echoes the requested mode when it is honoured, matching WebGL, and still returns `"normal"` when a mode genuinely falls back
- **`Body.destroy()` threw for a body with an ellipse collider** — `addShape` accepts an `Ellipse`, but `destroy()` had no branch to release one, so it fell through to the legacy pool, which throws for any class never registered with it. Because the body's `Bounds` was released first, the throw aborted teardown partway and left the body holding a recycled `Bounds` — surfacing later as an unrelated crash in the broadphase. Same trap the `Box3d` branch beside it already documented
- **`Body.destroy()` threw when called twice** — the shape loop was already a no-op the second time round, but the `Bounds` and vector fields are set to `undefined` rather than emptied, so re-releasing them threw "Instance is already in pool" and again aborted teardown partway. Reachable whenever a game destroys a body explicitly and the container teardown then destroys it again
- **A multi-shape sensor body was still pushed out of collisions** ([#1591](https://github.com/melonjs/melonJS/issues/1591)) — `body.isSensor` worked on a body with one shape and silently stopped working once a second was added: a sensor overlapping a solid body by 16px was displaced 8px. Bodies with 2+ shapes run an extra pass to resolve leftover overlaps at shape junctions (polylines, compound colliders), and that pass writes positions directly while checking only `isStatic` — undoing the main push-out's decision to leave a sensor alone. It now honours the sensor flag too. Affects any compound trigger volume or multi-part hitbox

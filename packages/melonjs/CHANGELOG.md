...
- **`device.focusOptions`** (#1486) — `{ auto: boolean }` object that finally lets user code disable the engine's "bring the window to the front on visibility restore" behaviour from outside the module. The flag itself lives in `device.ts`, but the public surface is a `Proxy`-wrapped object so writes (`me.device.focusOptions.auto = false`) survive the read-only ESM namespace binding. The Proxy setter also mirrors the new state into the legacy `device.autoFocus` `let`-binding so readers still calling `if (me.device.autoFocus)` keep seeing the current value. Both internal consumers (`device.initVisibilityEvents` and `pointerevent.ts`) now branch on `focusOptions.auto`.
...
- **`device.autoFocus`** (#1486, since 19.7.0). Use `device.focusOptions.auto` instead. The `let`-declared boolean export was never user-mutable through the ESM namespace import (`me.device.autoFocus = false` threw a `TypeError`); `focusOptions` is the object-wrapped equivalent whose property writes survive the namespace binding. The legacy export stays in sync with `focusOptions.auto` via a Proxy setter on the new export.

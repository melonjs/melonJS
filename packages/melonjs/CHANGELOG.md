...
### Fixed
- **WebGL `TextureCache` cross-batcher binding-tracker desync.** When the cache exhausted its unit pool and triggered a reset (either via `allocateTextureUnit` saturation or `resetUnitAssignments()`), only the current batcher's `boundTextures` map was cleared — every other batcher (`MeshBatcher`, idle `QuadBatcher`/`LitQuadBatcher`) kept stale `texture → unit` entries. Their next draw's `bindTexture2D` short-circuit (`texture === boundTextures[unit]`) then skipped the actual `gl.bindTexture` call, so the shader sampled whatever texture currently occupied the unit on the GPU. The highly visible symptom in mixed-batcher scenes — meshes coming out as **black silhouettes** (sampling a transparent HUD Text canvas left over from a quad-batcher unit assignment) and sprites/bullets coming out **pure white** (sampling the multi-material white-pixel fallback the mesh batcher had bound). The failure was intermittent and correlated with sustained gameplay because each `ParticleEmitter` allocates its own fallback canvas texture, draining the cache faster the more particles spawn. Fix: cache emits a new `event.GPU_TEXTURE_CACHE_RESET` after clearing its unit maps, and `MaterialBatcher` (base of `Mesh`/`Quad`/`LitQuad`) subscribes to it in `init` and drops its own bind tracking. The event is renderer-agnostic so a future WebGPU backend can reuse the same contract.
- WebGL color-attribute NaN canonicalization on Apple Metal / ANGLE. Two-part fix that came out of the same investigation: (a) the `MeshBatcher` color attribute switched from `UNSIGNED_BYTE × 4 normalized` to `FLOAT × 4` — bit patterns in `[0, 1]` can never form NaN, eliminating an upload-time canonicalization risk where the driver zeroed the alpha byte on any packed color with `A=0xFF` AND `R≥0x80`; (b) `Batcher.flush` and `QuadBatcher.flush` upload via `vertex.toUint8()` instead of `vertex.toFloat32()` so the same packed-color bytes (still in use by `Sprite` / `Particle` / `Primitive` paths) survive driver-side canonicalization on the upload path. `VertexArrayBuffer.toUint8()` and `bufferU8` are the new byte-view APIs added to support this. No behavior change on drivers that don't canonicalize.
- `Stage.reset` now re-applies the chosen camera class's `defaultSortOn` on every stage reset — not only when the stage explicitly overrides `cameraClass`. Previously a Camera3d application that went through the default loader screen (which pins `cameraClass: Camera2d` for its own use) saw `world.sortOn = "z"` persist into the next user stage instead of snapping back to `"depth"`. Symptom: distant meshes painted on top of nearer ones under perspective.
...
- `MeshBatcher` vertex stride grows from 6 floats to 9 floats per vertex (color promoted to `FLOAT × 4`). At realistic mesh complexity (sub-thousand vertices per Mesh per frame) the bandwidth difference is invisible; on a 50k-vertex scene it's a measurable but small ~2% of a 60fps frame budget. The previous packed-byte path was unreliable on Metal-backed drivers — see Fixed above.

...
- `Application.updateAverageDelta` is renamed **`lastUpdateDelta`** — it holds the measured wall-clock cost of the most recent logic step, and has never been an average: the exponential smoothing the name refers to was removed in 2015, a day after it was added. The old name remains as a deprecated alias and will be removed in 21.0.0. Note it is not the same quantity as `updateDelta`, which is the *simulated* time a step advances

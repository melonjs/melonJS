...
- WebGL: procedural `Light2d` rendering (closes #1430). New `Renderer.drawLight(light)` API replaces the per-light offscreen-canvas pipeline. The WebGL renderer renders each light as a single quad through a shared `Light2dEffect` fragment shader (linear radial falloff, matches the previous Canvas `createRadialGradient` exactly for visual parity); no per-light GL texture is allocated, all lights share one shader program. The Canvas renderer keeps the offscreen-canvas bake but moves it into a renderer-owned `WeakMap<Light2d, target>` cache that auto-invalidates on radius/color/intensity change. Light2d itself becomes pure data — no `CanvasRenderTarget`, no shader knowledge, no renderer reference.
- Light2d: `resize(radiusX, radiusY)` method. Updates the radii and the underlying bbox so `getBounds()` and `getVisibleArea()` track the new size. Fixes a latent bug where mutating `radiusX/Y` after construction left the rendered light stale while the cutout pass moved.
- WebGL: new `Light2dEffect` shader effect (`video/webgl/effects/light2d.js`). Standalone-usable: constructor accepts `{ color, intensity, radiusX, radiusY }`, plus `setColor`/`setIntensity`/`setRadii` setters. Used internally by `WebGLRenderer.drawLight`; available for any custom path that wants a procedural soft-circle quad.
...
- Light2d: stale gradient texture on radius/color/intensity change. The pre-#1430 implementation baked the gradient once at construction and re-used the same `CanvasRenderTarget` indefinitely; mutating `radiusX/Y`, `color`, or `intensity` left the rendered light out of sync with `getVisibleArea()` (the cutout pass) and the lit shader (which already used current values). The new `drawLight` cache auto-invalidates on property change, on both Canvas and WebGL.

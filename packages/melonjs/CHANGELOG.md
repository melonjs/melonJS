...
- Normal-map sprite lighting (SpriteIlluminator workflow, closes #1416). New `Sprite.normalMap` property accepts a paired normal-map image (or auto-detected from a `TextureAtlas` constructed with `{ normalMap }`). Sprites with a normal map render through the WebGL renderer's lit pipeline — the standard quad batcher's fragment shader samples the normal at each pixel and runs a Lambertian light loop over the active `Light2d` instances (`Stage._activeLights`), with up to 8 concurrent lights and a configurable `Stage.ambientLightingColor` floor. Sprites without a normal map render unchanged through a uniform-branch fast path (zero per-pixel cost). The Canvas renderer silently ignores `normalMap` and emits a one-shot console warning if any active light is uploaded. A new `Normal Map` example demonstrates the workflow with three procedurally-generated 3D orbs reacting to a single mouse-following light.

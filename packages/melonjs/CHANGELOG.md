...
### Added
- `renderer.prewarm()` builds the backend's built-in shader programs during `loader.preload()`, behind the loading screen, rather than on the frame that first draws them — called for you by `loader.preload()` — every call, not just the first — and controlled by the `prewarm` application setting. A game that never calls `preload()` is never warmed; call `renderer.prewarm()` directly in that case. A GPU backend does not finish a shader when it is handed the source; it finishes it the first time something is drawn with it, which puts the cost on the frame a scene appears. On WebGL this builds the mesh batchers' variants — fog, the four instance-record shapes and the ground-shadow pair, for the unlit tier and the lit one that inherits from it, so twenty-two programs; on WebGPU it compiles the WGSL module of every shader the loader holds, which is what makes declaring a level's effects alongside that level's assets worth doing. **On by default** (`prewarm: false` to opt out), and strictly additive: turned off, or with no `preload()` at all, every program is still built lazily exactly as before. The cost is paid where nothing is waiting on a frame — a purely 2D game links the mesh-tier programs it never binds, measured at about 60ms on a fast desktop GPU, inside a preload already showing a progress bar. Not a frame-rate feature: where a scene's first frame stalls, that is first-**submit** work (texture residency, buffer uploads, driver specialization) which compiling programs does not touch — this makes the shaders ready earlier, which matters on targets where linking is slow. Idempotent — a second call returns the same settled promise — and a context or device loss clears that memo, so a restored renderer warms again rather than reporting itself already done

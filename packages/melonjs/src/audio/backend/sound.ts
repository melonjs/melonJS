import { appleVendor } from "../../system/platform.ts";
import { loadBuffer } from "./buffer.ts";
import { cache } from "./cache.ts";
import { audioEngine } from "./core.ts";
import { installSpatialOnSound } from "./spatial.ts";
import {
	type EventListener,
...
class Sound {
	...
	init(o: SoundOptions): this {
		...
		this._webAudio = audioEngine.usingWebAudio && !this._html5;

		if (
			typeof audioEngine.ctx !== "undefined" &&
			audioEngine.ctx &&
			audioEngine.autoUnlock
		) {
			audioEngine._unlockAudio();
		}

		audioEngine.sounds.push(this);

		// Execute plugin hooks
		installSpatialOnSound(this, o);
	...
	play(sprite?: string | number, internal?: boolean): number | null {
		...
		if (this._webAudio && node && isGainNode(node)) {
			const playWebAudio = () => {
				...
				node.gain.setValueAtTime(vol, audioEngine.ctx!.currentTime);
				sound._playStart = audioEngine.ctx!.currentTime;
			...
			if (
				...
				audioEngine.ctx!.state !== "interrupted"
			) {
				playWebAudio();
			} else {
				this._playLock = true;
			...
			const playHtml5 = () => {
				...
				node.muted =
					sound._muted || this._muted || audioEngine._muted || node.muted;
				const volume = audioEngine.volume();
				node.volume = sound._volume * (typeof volume === "number" ? volume : 1);
				node.playbackRate = sound._rate;

				try {
					// typed as always returning a promise, but browsers before the
					// promise-returning spec return undefined
					const play = node.play() as Promise<void> | undefined;
			...
			if (node.readyState >= 3 || loadedNoReadyState) {
				// deferred a tick: on iOS a play() issued immediately after pause()
				// races the audio node, which pauses again right after starting
				// (goldfire/howler.js#1761)
				...
				const listener = () => {
					this._state = "loaded";
					playHtml5();
					node.removeEventListener(audioEngine._canPlayEvent, listener, false);
				};
				node.addEventListener(audioEngine._canPlayEvent, listener, false);
	...
	stop(id?: number, internal?: boolean): this {
		if (this._state !== "loaded" || this._playLock) {
			this._queue.push({
				event: "stop",
				action: () => {
					// forward `internal`: dropping it made the queued replay emit
					// events the caller had suppressed (goldfire/howler.js#1648)
		...
		for (let i = 0; i < ids.length; i++) {
			...
			if (sound) {
				...
				if (this._webAudio && sound._node && isGainNode(sound._node)) {
					...
					sound._node.muted = audioEngine._muted ? true : muted;
				}
	...
	volume(...args: unknown[]): number | this {
			...
			for (let i = 0; i < soundIds.length; i++) {
				...
				if (sound) {
					...
					if (
						...
						const volumeMultiplierOrGlobal = audioEngine.volume();
	...
	_stopFade(id: number): this {
		...
		if (sound && sound._interval) {
			if (this._webAudio && sound._node && isGainNode(sound._node)) {
				sound._node.gain.cancelScheduledValues(audioEngine.ctx!.currentTime);
			}
	...
	loop(...args: unknown[]): boolean | this {
		...
		for (let i = 0; i < ids.length; i++) {
			...
			if (sound) {
				...
				if (
					...
					sound._node.bufferSource.loop = loopVal ?? false;
					if (loopVal) {
			...
			if (typeof seekVal === "number" && seekVal >= 0) {
				...
				if (this._webAudio) {
					const realTime = this.playing(id)
						? audioEngine.ctx!.currentTime - sound._playStart
						: 0;
					const rateSeek = sound._rateSeek ? sound._rateSeek - sound._seek : 0;
					let position =
						sound._seek + (rateSeek + realTime * Math.abs(sound._rate));
					// keep the reported position inside the active loop: the JS end
					// timer can fire after the buffer has already restarted, which
					// otherwise reports a position past the loop end
					// (goldfire/howler.js#1776)
					if (sound._loop && typeof sound._stop === "number") {
						const loopStart = sound._start || 0;
						const loopDuration = sound._stop - loopStart;
						if (loopDuration > 0 && position >= sound._stop) {
	...
	unload(): null {
		...
		for (let i = 0; i < sounds.length; i++) {
			...
			if (!this._webAudio && node && isHTMLAudioElement(node)) {
				...
				if (loadFn) {
					node.removeEventListener(audioEngine._canPlayEvent, loadFn, false);
				}
				...
				audioEngine._releaseHtml5Audio(node);
			}
		...
		const index = audioEngine.sounds.indexOf(this);
		if (index >= 0) {
			audioEngine.sounds.splice(index, 1);
		}

		let remCache = true;
		for (let i = 0; i < audioEngine.sounds.length; i++) {
			if (
				audioEngine.sounds[i]._src === this._src ||
				(this._src as string).indexOf(audioEngine.sounds[i]._src as string) >= 0
			) {
				remCache = false;
				break;
			}
	...
	_emit(event: string, id?: number | null, msg?: string): this {
		const events = (this as unknown as Record<string, EventListener[]>)[
			`_on${event}`
		];
	...
	_soundById(id: number): Voice | null {
		for (let i = 0; i < this._sounds.length; i++) {
	...
	_refreshBuffer(sound: Voice): this {
		if (!sound._node || !isGainNode(sound._node) || !audioEngine.ctx) {
			return this;
		}

		sound._node.bufferSource = audioEngine.ctx.createBufferSource();
		const src =
			typeof this._src === "string"
				? this._src
				: Array.isArray(this._src) && this._src.length > 0
	...
	_cleanBuffer(node: any): this {
		// WebKit needs the buffer detached through a scratch buffer
		const isWebKit = appleVendor;

		if (!node.bufferSource) {
			return this;
		}

		if (audioEngine._scratchBuffer && node.bufferSource) {
			node.bufferSource.onended = null;
			node.bufferSource.disconnect(0);
			if (isWebKit) {
				try {
					node.bufferSource.buffer = audioEngine._scratchBuffer;
				} catch {
					// the node may already be disconnected; nothing to undo
				}
			}
		}

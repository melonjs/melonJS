...
/**
 * a button control that triggers SPACE key press/release events (jump)
 */
...
class Joypad extends UISpriteElement {
	...
	onDestroyEvent() {
		// release the registered pointer event
...
export class VirtualJoypad extends Container {
	...
	constructor() {
		...
		// keep the joypad pinned to the bottom corners on viewport resize.
		// Stored on `this` so `onDestroyEvent` can `event.off` it —
		// otherwise each PlayScreen reset adds another listener.
		this.onResize = (width: number, height: number) => {
			this.joypad.pos.set(50, height - 200, this.joypad.pos.z);

 ...
 * hit-region queries as the consumers. Matter bakes rotation into the body's
 * vertices rather than into the shape definitions it was given, so returning
...
describe("MatterAdapter — getBodyShapes() follows the body's rotation", () => {
	let world: World;
	...
	describe("several shapes, offset from the origin", () => {
		...
		it("leaves an inactive shape out of the simulation", () => {
			// `isActive: false` is the portable way to keep a shape out of
			// collision without removing it from the definition, and both the
			// builtin and planck honour it. Matter simulated it anyway, so the
			// same body collided differently depending on the backend.
			const r = new Renderable(400, 400, 100, 240);
			world.addChild(r);
			const live = new Polygon(0, 0, [
				new Vector2d(0, 0),
				new Vector2d(20, 0),
				new Vector2d(20, 20),
			]);
			const off = new Polygon(0, 0, [
				new Vector2d(60, 0),
				new Vector2d(80, 0),
				new Vector2d(80, 20),
			]);
			(off as unknown as { isActive: boolean }).isActive = false;
			adapter.addBody(r, { type: "static", shapes: [live, off] });
			expect(adapter.getBodyShapes(r)).toHaveLength(1);
			world.removeChildNow(r);
		});

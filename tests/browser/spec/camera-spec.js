...
describe("me.Camera2d", function () {
	...
	before(async () => {
		...
		await page.goto("http://localhost:8042/camera_test.html", {
			waitUntil: "load",
		});
	});
	...
	it("isVisible function test", async () => {
		...
		expect(
			await page.evaluate(() => camera.isVisible(renderable10x10)),
		).toEqual(true);
		...
		expect(
			await page.evaluate(() => infiniteCamera.isVisible(renderable10x10)),
		).toEqual(true);

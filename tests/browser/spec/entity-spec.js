...
describe("Entity", function () {
	...
	before(async () => {
		...
		await page.goto("http://localhost:8042/entity_test.html", {
			waitUntil: "load",
		});
	});
	...
	it("has an empty set of shapes", async () => {
		expect(
			await page.evaluate(() => globalThis.entity.body.shapes.length),
		).toEqual(0);
	});
	...
	it("has a second shape", async () => {
		await page.evaluate(() => {
			globalThis.entity.body.addShape(
				defaultRectShape.clone().setShape(-10, -10, 32, 64),
			);
		});
		expect(
			await page.evaluate(() => globalThis.entity.body.shapes.length),
	...
	it("has the correct entity geometry: C", async () => {
		expect(
			await page.evaluate(() => {
				return (
					globalThis.entity.pos.x === 0 &&
					globalThis.entity.pos.y === 0 &&
					globalThis.entity.width === 32 &&
					globalThis.entity.height === 64
				);
			}),
		).toEqual(true);
	});
	...
	it("moves properly", async () => {
		expect(
			await page.evaluate(() => {
				...
				return (
					globalThis.entity.pos.x === 120 && globalThis.entity.pos.y === 150
				);
			}),
		).toEqual(true);
	});

...
describe("me.Container", () => {
    ...
    beforeEach(async () => {
        await page.evaluate(() => {
            window.container = new me.Container(0, 0, 100, 100, true);
        });
    });
    ...
    describe("isAttachedToRoot", () => {
        it("should return true", async () => {
            expect(
                await page.evaluate(() => container.isAttachedToRoot())
            ).toEqual(true);
        });

        it("a new container should not be root", async () => {
            expect(
                await page.evaluate(() => {
                    const secondContainer = new me.Container();
                    return secondContainer.isAttachedToRoot();
                })
            ).toEqual(false);
        });

        it("a new container when attached to game world should find root", async () => {
            expect(
                await page.evaluate(() => {
                    const secondContainer = new me.Container();

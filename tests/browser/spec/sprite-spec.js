...
describe("me.Sprite", function () {
	...
	before(async () => {
		...
		await page.goto("http://localhost:8042/sprite_test.html", {
			waitUntil: "load",
		});
	});
	...
	describe("isAttachedToRoot", function () {
		...
		it("me.Sprite addAnimation should return the correct amount of frame", async () => {
			expect(
				await page.evaluate(() => sprite.addAnimation("test", [0, 1])),
			).toEqual(2);
			expect(
				await page.evaluate(() =>
					sprite.addAnimation("test2", [0, 1, 0, 1, 0]),
				),
			).toEqual(5);
		});
		...
		it("me.Sprite isCurrentAnimation allows to verify which animation is set", async () => {
			expect(
				await page.evaluate(() =>
					sprite.addAnimation("yoyo", [1, 0, 1, 0], 60),
				),
			).toEqual(4);
